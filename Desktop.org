...
* Environment
:PROPERTIES:
:header-args:sh: :tangle ~/.config/profile :mkdirp yes
:END:
...
These are the guix channels I currently use. Flatwhatson's guix [[https://github.com/flatwhatson/guix-channel][channel]] offers some interesting packages, for example different Emacs variants like a =pgtk= or =native-comp= build.
...
#+begin_src sh :tangle ~/.local/share/bin/update-manifest :tangle-mode (identity #o755) :shebang "#!/bin/sh" :mkdirp yes
if [ -n "$1" ]; then
...
*** Run Emacs
#+begin_src sh :tangle ~/.local/share/bin/run-emacs :tangle-mode (identity #o755) :shebang "#!/bin/sh" :mkdirp yes
...
*** Run WSL
#+begin_src sh :tangle ~/.local/share/bin/run-wsl :tangle-mode (identity #o755) :shebang "#!/bin/sh" :mkdirp yes
...
I want to configure some applications in other files (for example development tools like [[https://www.npmjs.com/][npm]]). In order for this to work I need to source their configuration files through my =profile=. Therefore, I create =$XDG_CONFIG_HOME/profile.d= and source every file in it.
...
** ZShell
:PROPERTIES:
:header-args:sh: :tangle ~/.config/zsh/.zshrc :mkdirp yes
:END:
...
** Sway
:PROPERTIES:
:header-args:conf: :tangle ~/.config/sway/config :mkdirp yes
:END:
