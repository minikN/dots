...
The idea is to export the theme's current colors, format them in css as save them to =xdg-config-home/waybar/colors.css=. In my waybar config I can then =@import 'colors.css'= to use them. All that is left to do is reload the sway config with =swaymsg reload=.
