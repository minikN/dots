...
* Manifest
Some packages require external packages (binaries, fonts, ...). I use a Guix profile called =Emacs= for that purpose. Its manifest file will be tangled to =$XDG_CONFIG_HOME/guix/manifests/emacs.scm= and can be installed with =update-manifest emacs=.
