...
| db-data-home       | =/home/db/.local/share=           |
| db-config-home     | =/home/db/.config=                |
...
| Type         | Content                              |
|--------------+--------------------------------------|
| Guix Profile | =guix-manifest-home/emacs.scm=       |
| Guix Profile | =guix-manifest-home/development.scm= |
...
Well, what is org-mode good for if not tangling files? Just kidding. Generally speaking, the source blocks of this file will be tangled to =config.el= while I'll tangle bits and pieces to =init.el= and =packages.el= where needed.
...
** Environment
One of the first things I like to declare are some variables and constants describing the environment we are running in.
...
(use-package! xdg
  ...
  (defvar db-cache-chome
    (or (xdg-cache-home) "~/.cache")
    "The cache location according to the XDG specification.")
...
(map! :leader
      ...
      :desc "Open literate config"              "p"     (cmd! (doom-project-browse (file-name-as-directory db-lit-config-home))
...
When using the modus themes, I like to disable DOOM's =solaire-mode=.
...
The idea is to export the theme's current colors, format and save them as css to =db-config-home/waybar/colors.css=. In my waybar config I can then =@import= them. All that is left to do is reload the sway config with =swaymsg reload=.
...
#+begin_src sh :tangle (concat (cdr (assoc 'db-config-home file-paths)) "/npm/npmrc") :mkdirp yes
...
This configuration is for the development environment to work properly. All the npm packages I need by default (like =eslint= or =typescript=) are installed in =db-data-home/npm=, therefore, I can create a environment wrapper that checks if all the tools needed are installed and does so if not.
...
#+begin_src emacs-lisp :noweb-ref config
 (setq lsp-eslint-node "node-env node"
       ...
       `("node-env"
         "node"
         ,(concat db-data-home "/npm/vscode-eslint/server/out/eslintServer.js")
         "--stdio"))
#+end_src
...
# eval: (map-put file-paths 'db-config-home (or (getenv "XDG_CONFIG_HOME") "~/.config"))
# eval: (map-put file-paths 'db-data-home (or (getenv "XDG_DATA_HOME") "~/.local/share"))
# eval: (map-put file-paths 'profiled-home (concat (cdr (assoc 'db-config-home file-paths)) "/profile.d"))
# eval: (map-put file-paths 'npm-config-home (concat (cdr (assoc 'db-config-home file-paths)) "/npm"))
# eval: (map-put file-paths 'doomdir (concat (cdr (assoc 'db-config-home file-paths)) "/doom"))
# eval: (map-put file-paths 'guix-config-home (concat (cdr (assoc 'db-config-home file-paths)) "/guix"))
# eval: (map-put file-paths 'guix-manifest-home (concat (cdr (assoc 'guix-config-home file-paths)) "/manifests"))
# eval: (map-put file-paths 'bindir (concat (cdr (assoc 'db-data-home file-paths)) "/bin"))
